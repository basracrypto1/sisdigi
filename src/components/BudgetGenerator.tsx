...
export const BudgetGenerator: React.FC = () => {
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('RINCIAN ANGGARAN BIAYA (RAB) AI', 105, 20, { align: 'center' });
    doc.setFontSize(12);
    doc.text(`Judul: ${result.judul}`, 14, 35);
    doc.text(`Jenis: ${result.jenis}`, 14, 42);
    doc.text(`Total Anggaran: ${formatCurrency(result.total_anggaran)}`, 14, 49);
    ...
    doc.autoTable({
      startY: 60,
      head: [['No', 'Item', 'Qty', 'Satuan', 'Harga Satuan', 'Subtotal']],
      body: body,
      foot: [['', 'TOTAL AKHIR', '', '', '', formatCurrency(totalActual)]],
      theme: 'grid',
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className="p-3 bg-red-500 text-white rounded-2xl hover:brightness-110 shadow-lg shadow-red-500/20 transition-all flex items-center gap-2">
                       <Save className="w-4 h-4" />
                       <span className="text-[9px] font-black uppercase tracking-widest pr-1">Simpan Draft</span>
                    </button>
                  </div>
                </div>
